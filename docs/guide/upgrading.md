...
| Feature | Summary |
|---|---|
| [`SlugConfig` fluent API](/guide/configuration#via-the-fluent-slugconfig-api) | A `slugConfig()` method as an alternative to `#[Slugify]`, for closures and conditional logic the attribute can't express |
| [ID-anchored slugs (`appendId`)](/guide/id-anchored-slugs) | `#[Slugify(appendId: true)]` route keys become `{slug}-{id}` with `308` canonical redirects |
| [Laravel Boost skill](/guide/features#laravel-boost-skill) | Bundled `slugify-development` skill, auto-discovered by Boost |
...
If you are coming from `spatie/laravel-sluggable` (v3 or v4) rather than from v1.x of this package, see the [Migrating from Spatie](/guide/migrating-from-spatie) guide instead.